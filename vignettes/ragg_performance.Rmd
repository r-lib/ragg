...
ragg and cairo_png produce files of around 2Kb, whereas cairo
...
Plotting rasters is whicked fast it appears. For non-rotated rasters ragg is a
...
supported by Cairo and PDF in the base installation (Xlib will do nothing and
thus appear to be very fast). ragg will strive to keep up with the new features 
and though it may lack behind a bit as development takes time.
...
We can see that ragg has fares much better than cairo when it comes to rendering
...
### Masks
Masks allows you to define the alpha level of a layer based on information from
...
still quite a bit faster, but cairos mask performance is faster than raggs 
...
different patterns will have considerable overheads as multiple grobs needs to 
be created.
