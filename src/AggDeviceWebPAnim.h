 ...
 public:
  AggDeviceWebPAnim(const char* fp, int w, int h, double pointsize,
      ...
      : AggDevice<PIXFMT>(fp, w, h, pointsize, background, res, scaling,
        ...
        verbose_(LOGICAL(Rf_GetOption1(Rf_install("verbose")))[0]),
    ...
    if (verbose_) {
      Rprintf("WebP animation: %dx%d, %dms delay, %d loops, %s\n",
              w, h, delay_ms_, loop_count_, lossy_ ? "lossy" : "lossless");
    }
  }
  ...
  bool savePage() {
    ...
    try {
      ...
      if (wr.size > 0 && wr.mem != nullptr) {
        ...
        if (verbose_) {
          Rprintf("WebP frame %zu: encoded %zu bytes\n",
                  frames_.size(), wr.size);
        }
      } else {
  ...
  void close() {
    ...
    for (size_t i = 0; i < frames_.size(); ++i) {
      ...
      WebPMuxError frame_err = WebPMuxPushFrame(mux_.get(), &finfo, 1);
      if (frame_err != WEBP_MUX_OK) {
        Rf_error("Failed to push WebP frame %zu/%zu: %s (%zu bytes)", 
                 i + 1, frames_.size(), mux_error_name(frame_err), raw.size());
      }
    }

    try {
      auto out = make_webp_data();
      WebPMuxError mux_err = WebPMuxAssemble(mux_.get(), out.get());
      if (mux_err != WEBP_MUX_OK) {
        Rf_error("WebP mux assemble failed: %s (%zu frames)", mux_error_name(mux_err), frames_.size());
      }
      ...
      if (verbose_) {
        Rprintf("WebP animation complete: %zu frames, %zu bytes written to %s\n",
                frames_.size(), out->size, this->file.c_str());
      }
